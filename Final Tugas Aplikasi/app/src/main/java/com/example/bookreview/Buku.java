...
public class Buku extends AppCompatActivity implements View.OnClickListener,NavigationView.OnNavigationItemSelectedListener {
    ...
    private String Isbn, JudulBuku, NamaPengarang;
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.bt_TambahB :
                ...
                JudulBuku = et_JudulBuku.getText().toString().trim();
                NamaPengarang = et_NamaPengarang.getText().toString().trim();
                if (TextUtils.isEmpty(Isbn)){
                    isEmpty=true;
                    et_Isbn.setError("Field ini tidak boleh kosong");
                }
                if (TextUtils.isEmpty(JudulBuku)){
                    isEmpty=true;
                    et_JudulBuku.setError("Field ini tidak boleh kosong");
                }
                if (TextUtils.isEmpty(NamaPengarang)){
                    isEmpty = true;
                    et_Isbn.setError("Field ini tidak boleh kosong");
                }
                ...
                if (!isEmpty){
                    final PojoBuku PB = new PojoBuku();
                    PB.setIsbn(et_Isbn.getText().toString().trim());
                    PB.setJudul_buku(et_JudulBuku.getText().toString().trim());
                    PB.setNama_pengarang(et_NamaPengarang.getText().toString().trim());
                    PB.setKategori_buku(s_Kategori.getSelectedItem().toString());
                    dRef.child("Buku").orderByChild("Judul").equalTo(PB.getJudul_buku()).addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            if (dataSnapshot.exists()==false)  {
                                dRef.child("Buku").orderByChild("ISBN").equalTo(PB.getIsbn()).addListenerForSingleValueEvent(new ValueEventListener() {
                                    @Override
                                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                        if (dataSnapshot.exists()==true){
                                            Toast.makeText(getApplicationContext(), "ISBN buku telah tedaftar", Toast.LENGTH_SHORT).show();
                                        }else {
                                            dRef.child("Buku").child(JudulBuku).child("ISBN").setValue(PB.getIsbn());
                                            dRef.child("Buku").child(JudulBuku).child("Judul").setValue(PB.getJudul_buku());
                                            dRef.child("Buku").child(JudulBuku).child("Kategori").setValue(PB.getKategori_buku());
                                            dRef.child("Buku").child(JudulBuku).child("Pengarang").setValue(PB.getNama_pengarang());
                                            uploadGambar();
